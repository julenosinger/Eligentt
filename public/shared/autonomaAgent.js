...
(function(){
  ...
  function _getAgentProvider(){
    ...
    try {
      if(typeof getCachedProvider === 'function') return getCachedProvider('https://rpc.mainnet.arc.io');
    } catch(e){}
  ...
  function checkAlerts(){
    ...
    try {
      var balFloat = null;
      // On-chain balance: use Circle Wallet address as canonical agent address
      try {
        var _circleAddr = (typeof CircleAgent !== 'undefined' && CircleAgent.getCachedAddress) ? CircleAgent.getCachedAddress() : null;
        if(_circleAddr && typeof walletAddress !== 'undefined' && walletAddress){
          var agentAddr = _circleAddr;
  ...
  function getAgentIdentityCard(R){
    ...
    // Agent identity: Circle Wallet is canonical — AWM state summary is replaced by CircleAgent
    var _circleIdentityAddr = null;
    try { if(typeof CircleAgent!=='undefined'&&CircleAgent.getCachedAddress) _circleIdentityAddr=CircleAgent.getCachedAddress(); } catch(_e){}
    var state = _circleIdentityAddr ? { walletAddress: _circleIdentityAddr } : null;
