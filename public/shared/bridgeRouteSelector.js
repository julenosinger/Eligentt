...
(function(){
  ...
  function render(col) {
    ...
    // Patch executeBridgeOrTurbo — CCTP v2 is default, Turbo opt-in via pill
    if (typeof window.executeBridgeOrTurbo === 'function' && !window.__brRouteWrapped) {
      window.executeBridgeOrTurbo = function() {
        if (_selectedRoute === 'turbo' && typeof bridgeToIdx !== 'undefined' && typeof CHAINS !== 'undefined') {
          var tc = CHAINS[bridgeToIdx];
          if (tc && tc.id === 'Arc_Testnet' && typeof executeTurboBridge === 'function') return executeTurboBridge();
        }
