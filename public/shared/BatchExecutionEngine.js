/**
 * Batch Execution Engine — Auto-Execution Layer for Multisend & Batch Intents
 * ADDITIVE module — zero modifications to existing systems.
 *
 * Architecture:
 *   AI Wallet → creates validated intents → ScheduleEngine entry
 *   BatchExecutionEngine → auto-pickup for instant multisend/batch intents
 *   → triggers existing _agentExecuteMultiSend via window
 *   → tracks progress → generates reports → updates history
 *
 * Engines (bundled, additive):
 *   BatchExecutorEngine      — auto-pickup + execution trigger
 *   BatchProgressTracker     — per-recipient progress tracking
 *   BatchReportEngine        — post-execution reporting
 *
 * ONLY activates for:
...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════
     BATCH EXECUTOR ENGINE — Auto-Pickup & Execute
     ══════════════════════════════════════════════════════════════ */
   ...
   * Uses the EXISTING _agentExecuteMultiSend — zero duplicated business logic.
   */
  async function executeBatchSchedule(sched) {
    ...
    try {
      ...
      execEntry.progressLabel = 'Batch completed — ' + params.addresses.length + ' recipients';
   ...
   * Only retries for RPC/temporary failures — never for auth/permission/balance failures.
   */
  ...
  function getProgressHTML(execEntry) {
    ...
    return '<div style="border:1px solid ' + (execEntry.status === 'running' || execEntry.status === 'broadcasting' ? 'rgba(167,139,250,.4)' : 'var(--border)') + ';border-radius:7px;padding:9px;margin-bottom:7px;background:rgba(0,0,0,.15)">' +
      ...
      '<span>' + pct + '%' + (execEntry.startedAt ? ' · ' + formatElapsed(Date.now() - execEntry.startedAt) : '') + '</span></div>' +
  ...
  /* ══════════════════════════════════════════════════════════════
     REPORT ENGINE — BatchReportEngine
     ══════════════════════════════════════════════════════════════ */
    ...
    try {
      if (typeof ExecutionHistory !== 'undefined') {
        ExecutionHistory.recordExecution({
          ...
          displayText: 'Batch ' + params.addresses.length + ' recipients — ' + params.total + ' ' + params.token
        });
      }
    } catch (_e) {}
    try {
  ...
  /* ══════════════════════════════════════════════════════════════
     AUTO-POLL LOOP — checks for new due batch schedules
     ══════════════════════════════════════════════════════════════ */
  async function poll() {
    if (_busy) return;
  ...
  function onScheduleCreated(detail) {
    ...
    // Immediate pickup — short delay to allow validation completion
    setTimeout(function () {
      poll();
    }, 1200);
  }
  ...
  /* ══════════════════════════════════════════════════════════════
     DIRECT EXECUTION API — for AI Wallet to call directly
     ══════════════════════════════════════════════════════════════ */
  ...
  /* ══════════════════════════════════════════════════════════════
     INIT — Background poll + event listener
     ══════════════════════════════════════════════════════════════ */
  function init() {
    load();
