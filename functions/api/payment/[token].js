...
export async function onRequestGet(context) {
  try {
    ...
    if (!token) return new Response(JSON.stringify({ error: 'Token required' }), { status: 400, headers: CORS_HEADERS });

    const KV = context.env.PAYMENT_LINKS;
    if (!KV) return new Response(JSON.stringify({ error: 'Storage unavailable' }), { status: 503, headers: CORS_HEADERS });

    const raw = await KV.get(token);
    if (!raw) return new Response(JSON.stringify({ error: 'Payment link not found' }), { status: 404, headers: CORS_HEADERS });
    ...
    return new Response(JSON.stringify({ ok: true, link }), { status: 200, headers: CORS_HEADERS });
  } catch (e) {
...
export async function onRequestPost(context) {
  try {
    ...
    if (link.status === 'Paid') {
      return new Response(JSON.stringify({ error: 'Payment already completed' }), { status: 409, headers: CORS_HEADERS });
    }
    if (link.status === 'Expired' || (link.expiresAt && new Date(link.expiresAt) < new Date())) {
      return new Response(JSON.stringify({ error: 'Payment link expired' }), { status: 410, headers: CORS_HEADERS });
    }
    if (link.status === 'Disabled') {
      return new Response(JSON.stringify({ error: 'Payment link disabled' }), { status: 403, headers: CORS_HEADERS });
    }
    ...
    if (!txHash || !/^0x[0-9a-fA-F]{64}$/.test(txHash)) {
      return new Response(JSON.stringify({ error: 'Invalid transaction hash' }), { status: 400, headers: CORS_HEADERS });
    }
    ...
    if (!receipt) {
      return new Response(JSON.stringify({ error: 'Transaction not found on-chain. It may still be pending.' }), { status: 422, headers: CORS_HEADERS });
    }
    if (receipt.status !== 1) {
      return new Response(JSON.stringify({ error: 'Transaction failed on-chain (reverted)' }), { status: 422, headers: CORS_HEADERS });
    }
    ...
    if (feeAmount > 0) {
      if (!feeTxHash || !/^0x[0-9a-fA-F]{64}$/.test(feeTxHash)) {
        return new Response(JSON.stringify({ error: 'Fee transaction hash required for paid links with protocol fee' }), { status: 400, headers: CORS_HEADERS });
      }

      const feeReceipt = await provider.getTransactionReceipt(feeTxHash);
      if (!feeReceipt) {
        return new Response(JSON.stringify({ error: 'Fee transaction not found on-chain' }), { status: 422, headers: CORS_HEADERS });
      }
      if (feeReceipt.status !== 1) {
        return new Response(JSON.stringify({ error: 'Fee transaction failed on-chain (reverted)' }), { status: 422, headers: CORS_HEADERS });
      }
